...
def main():
    ...
    logging.info("Finalizer worker started...")

    for msg in consumer:
        event = msg.value
        doc_id = event["document_id"]

        logging.info(f"Finalizer received event on {msg.topic} for doc={doc_id}")

        ensure_progress_row(conn, doc_id)

        if msg.topic == "document.s3_deleted":
            mark_step_complete(conn, doc_id, "s3")
        elif msg.topic == "document.es_deleted":
            mark_step_complete(conn, doc_id, "es")

        state = get_progress(conn, doc_id)
        logging.info(f"Progress state: {state}")

        if state["s3_done"] and state["es_done"]:
            logging.info(f"All steps completed for doc={doc_id}. Finalizing deletion...")

            delete_document_metadata(conn, doc_id)
